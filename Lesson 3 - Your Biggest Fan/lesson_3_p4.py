...
# Setup servo PWM
servo_pwm = pwmio.PWMOut(board.GP0, duty_cycle=0, frequency=50)
...
while True:
    if not speed_button.value:  # Button 1 pressed
        ...
        print(f"Setting servo angle to {servo_angles[speed_selector]}")
        servo_motor.angle = servo_angles[speed_selector]
